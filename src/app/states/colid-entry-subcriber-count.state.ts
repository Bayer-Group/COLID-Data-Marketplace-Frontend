...
export class ColidEntrySubscriberCountState {
  constructor(
    private colidEntryApiService: ColidEntryApiService,
    private store: Store
  ) {}

  @Selector()
  public static getSubscriptionNumbers(
    state: ColidEntrySubscriberCountStateModel
  ) {
  ...
  FetchColidEntrySubscriptionNumbers(
    { patchState }: StateContext<ColidEntrySubscriberCountStateModel>,
    {}: FetchColidEntrySubscriptionNumbers
  ) {
    const pidUriList = this.store
      .selectSnapshot(SearchState.getSearchResult)
      .hits.hits.map((x) => decodeURIComponent(x.id));

    if (pidUriList != null && pidUriList.length > 0) {
      this.colidEntryApiService
