...
import { policyRequest } from '../shared/models/resourcePolicies/policy-request';
...
export class ResourcePoliciesState {
    constructor(private ironMountainApiService: IronMountainService, private store: Store) {
    }
    ...
    FetchResourcePolicies({ patchState }: StateContext<ResourcePoliciesStateModel>, {searchResult}: FetchResourcePolicies) {
       /* patchState({
            loading: true, 
        });
        var dataCategoryResources = searchResult.hits.hits.filter(x => x.source[Constants.Metadata.HasDataCategory] )  
        if (dataCategoryResources.length >= 1){
            ...
            dataCategoryResources.map(x=> {
                var dict = new policyRequest(
                    ...
                    x.source[Constants.Metadata.HasDataCategory].outbound.map(x=> x.uri),
                    x.source[Constants.Metadata.HasCountryContext] ? x.source[Constants.Metadata.HasCountryContext].outbound.map(x=> x.uri) : []
                );  
                policyRequestValues.push(dict)
            })
         
            this.ironMountainApiService.getIronMountainResourcePolicies(policyRequestValues).subscribe(cs => {
                patchState({
                    loading: false,
                    resourcePolicies: cs
                });
