...
export class RemoveSearchFilterDataMarketplace {
    static readonly type = '[SearchFilterDataMarketplace] Remove SearchFilter DataMarketplace';
    constructor(public searchFilterDataMarketplaceDto: SearchFilterDataMarketplaceDto) { }
}

export class FetchSearchFilterDataMarketplace {
    static readonly type = '[SearchFilterDataMarketplace] Fetch Search Filter Data Market place';
    constructor() { }
}
...
export class UserInfoState {
    ...
    SetLastLoginDataMarketplace({ getState, patchState }: StateContext<UserInfoStateModel>, { }: SetLastLoginDataMarketplace) {
        ...
        if (user != null) { // only for first login of current user and it's creation
            const loginDate = new Date();
            return this.userInfoApiService.setLastLoginDataMarketplace(user.id, loginDate).pipe(tap(res => {
    ...
    CreateDefaultSearchFilterDataMarketplace({ patchState, getState }: StateContext<UserInfoStateModel>, { searchFilterDataMarketplaceDto }: CreateDefaultSearchFilterDataMarketplace) {
        ...
        return this.userInfoApiService.addSearchFilterDataMarketplace(user.id, searchFilterDataMarketplaceDto).subscribe((res: UserDto) => {
            if(res != null) {
                user.searchFiltersDataMarketplace = res.searchFiltersDataMarketplace;
                patchState({
                    user: user
                ...
                if(res.searchFiltersDataMarketplace != null && res.searchFiltersDataMarketplace.length > 0) {
                    let newSearchFilterDataMarketplace : any = res.searchFiltersDataMarketplace.filter(dto => dto.name === searchFilterDataMarketplaceDto.name)[0];

                    if(newSearchFilterDataMarketplace != null) {
                        return this.userInfoApiService.setDefaultSearchFilterDataMarketplace(user.id, newSearchFilterDataMarketplace.id).subscribe((res: UserDto) => {
                            user.defaultSearchFilterDataMarketplace = res.defaultSearchFilterDataMarketplace;
                            patchState({
                                user: user
                            });
                        });
    ...
    RemoveSearchFilterDataMarketplace({ patchState, getState }: StateContext<UserInfoStateModel>, { searchFilterDataMarketplaceDto }: RemoveSearchFilterDataMarketplace) {
        const user = getState().user;
        return this.userInfoApiService.removeSearchFilterDataMarketplace(user.id, searchFilterDataMarketplaceDto.id).pipe(tap((res: UserDto) => {
            user.searchFiltersDataMarketplace = res.searchFiltersDataMarketplace;
            patchState({
                user: user
            });
    ...
    fetchSearchFilterDataMarketplace({ patchState,getState }: StateContext<UserInfoStateModel>, {  }: FetchSearchFilterDataMarketplace) {
       var user = getState().user
       return this.userInfoApiService.getSearchFiltersDataMarketplace(user.id)
            .pipe(
    ...
    AddStoredQueryToSearchFiltersDataMarketplace({ patchState, getState }: StateContext<UserInfoStateModel>, { storedQueryDto }: AddStoredQueryToSearchFiltersDataMarketplace) {
        const user = getState().user; 
       if(storedQueryDto.searchFilterDataMarketplaceId==0){
            var length=user.searchFiltersDataMarketplace.length;
            storedQueryDto.searchFilterDataMarketplaceId=length>0?user.searchFiltersDataMarketplace[length-1].id:0;
        }
        return this.userInfoApiService.addStoredQueryDataMarketplace(user.id, storedQueryDto).pipe(tap((res: SearchFilterDataMarketplaceDto) => {
           const index = user.searchFiltersDataMarketplace.findIndex(data =>data.id===res.id);
            user.searchFiltersDataMarketplace[index].storedQuery =res.storedQuery
            patchState({
                user: user
            });
        }));
    ...
    RemoveStoredQueryToSearchFiltersDataMarketplace({ patchState, getState }: StateContext<UserInfoStateModel>, { searchFilterDataMarketplaceId }: RemoveStoredQueryToSearchFiltersDataMarketplace) {
        const user = getState().user; 
        return this.userInfoApiService.removeStoredQueryDataMarketplace(user.id, searchFilterDataMarketplaceId).pipe(tap((res: SearchFilterDataMarketplaceDto) => {
            var index=user.searchFiltersDataMarketplace.findIndex(x=>x.id==res.id);
            if(index>-1){
