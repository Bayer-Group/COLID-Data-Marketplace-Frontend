...
export class FilterBoxItemCheckboxHierarchyComponent implements OnInit {
  ...
  activeCategoryTab$: Observable<number>;
  ...
  tabChanged(tabChangeEvent: MatTabChangeEvent): void {
    this.store
      .dispatch(new ToggleCategoryFilterTab(tabChangeEvent.index))
      .subscribe();
    this.emitLastChange();
  }
  ...
  initTree() {
    ...
    this.treeControl = new FlatTreeControl<CheckboxHierarchyDTO>(
      this.getLevel,
      this.isExpandable
    );
  ...
  AddAllNonInstantiableNodes(nodeList: CheckboxHierarchyDTO[]) {
    nodeList.forEach((node) => {
      ...
      if (
        node.id != null &&
        !this.treeDataNodeList.some((x) => x.id == node.id)
      ) {
  ...
  isExpandable = (_) => true; //node.hasChild;

  getChildren = (node: CheckboxHierarchyDTO): CheckboxHierarchyDTO[] =>
    node.children;
  ...
  descendantsAllSelected(node: CheckboxHierarchyDTO): boolean {
    ...
    const descendants = this.treeControl
      .getDescendants(node)
      .filter((x) => x.instantiable);
    if (descendants.length > 0) {
      const descAllSelected = descendants.every((child) =>
        this.activeNodes.includes(child.id)
      );
  ...
  categoryAllSelected(node: CheckboxHierarchyDTO): boolean {
    ...
    const descAllSelected = node.children.every((child) =>
      this.activeNodes.includes(child.id)
    );
  ...
  rewriteValues() {
    if (this.dataSource) {
      this.dataSource.data = this.filterIndexedCheckboxHierarchyItems(
        this.checkboxHierarchyTreeData,
  ...
  filterIndexedCheckboxHierarchyItems(
    treeData: CheckboxHierarchyDTO[],
    aggregationBuckets: AggregationBucket[]
  ): CheckboxHierarchyDTO[] {
    if (treeData) {
      var indexedItems = treeData.filter((t) =>
        aggregationBuckets.some((a) => a.key === t.name)
      );
      indexedItems.forEach((i) => {
        if (i.hasChild) {
          i.children = this.filterIndexedCheckboxHierarchyItems(
            i.children,
            aggregationBuckets
          );
        }
      });
      const test = indexedItems.filter(
        (ie) => (ie.children.length > 0 && ie.hasChild) || !ie.hasChild
      );
      return test;
    }
  }
  ...
  filterDuplicatesAndRemoveChildNodes(): string[] {
    ...
    this.treeDataNodeList.forEach((e) => {
      if (this.activeNodes.includes(e.id)) {
        if (this.activeTab == 0) {
          if (e.isCategory) {
            var parent = this.treeDataNodeList.find(
              (x) => x.name == e.parentName
            );
