...
export class SimilarityModalComponent implements OnInit {
  docs: any[] = []
  threshold: number = 0
  ...
  constructor(public dialogRef: MatDialogRef<SimilarityModalComponent>,
  ...
  loadSimilarResources() {
    this.similarityService.loadSimilarityResourceByResource(this.generatePayload()).subscribe((data) => {
  ...
  private generatePayload() {
    ...
    payload.properties[Constants.Metadata.HasPidUri] = this.source[Constants.Metadata.HasPidUri].outbound[0].value

    if (typeof this.source[Constants.Metadata.HasResourceDefinition] != 'undefined') {
      payload.properties[Constants.Metadata.HasResourceDefinition] = this.source[Constants.Metadata.HasResourceDefinition].outbound[0].value
    } else {
      payload.properties[Constants.Metadata.HasResourceDefinition] = '';
    }

    if (typeof this.source[Constants.Metadata.HasLabel] != 'undefined') {
      payload.properties[Constants.Metadata.HasLabel] = this.source[Constants.Metadata.HasLabel].outbound[0].value
    } else {
      payload.properties[Constants.Metadata.HasLabel] = ''
    }

    if (typeof this.source[Constants.Metadata.EntityType] != 'undefined') {
      payload.properties[Constants.Metadata.EntityType] = this.source[Constants.Metadata.EntityType].outbound[0].uri
    } else {
      payload.properties[Constants.Metadata.EntityType] = ''
    }

    if (typeof this.source[Constants.Metadata.HasPIDEditorialNote] != 'undefined') {
      payload.properties[Constants.Metadata.HasPIDEditorialNote] = this.source[Constants.Metadata.HasPIDEditorialNote].outbound[0].value
    } else {
