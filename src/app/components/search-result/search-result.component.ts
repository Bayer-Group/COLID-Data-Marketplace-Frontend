...
export class SearchResultComponent implements OnInit, OnDestroy, OnChanges {
  ...
  ngOnInit() {
    ...
    // set flag for next review date
    if (
      this.details.find((x) => x.key == Constants.Metadata.HasNextReviewDueDate)
    ) {
    ...
    // set flag for broken contacts and broken endpoints
    const invalidContacts = new Set<string>();
    if (
      this.details.find((x) => x.key == Constants.Metadata.HasBrokenDataSteward)
    ) {
      this.details
  ...
  findPolicies(event) {
    this.preventOpeningResultDetails(event);
    const pidUri = decodeURIComponent(this._result.id);
    this.dialog.open(ResourcePoliciesComponent, {
      data: {
        pidUri: pidUri,
      },
    });
