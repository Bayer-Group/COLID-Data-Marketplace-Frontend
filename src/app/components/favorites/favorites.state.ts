...
export class RemoveEntry {
  ...
  constructor(public userId: string, public pidUri: string, public favoriteListId: string, public favoritesListEntryId: string) { }
}

export class SaveFavoriteListEntry {
  static readonly type = "[Favorite] Save favorite list entry";
  constructor(public userId: string, public favoriteListId: string, public favoriteListEntryId: string, public pidUri: string, public personalNote: string) { }
}

export class SaveFavoriteList {
  static readonly type = "[Favorite] Save favorite list";
  constructor(public usedId: string, public favoriteListId: string, public name: string){}
}

export class SetEntryDetailsMetadata {
  static readonly type = "[Favorite] Show entry details";
  constructor(public favoritesListId: string, public favoriteEntryId: string, public pidUri: string, public personalNote: string) { }
}
...
export class FavoritesStateModel {
  ...
  favoriteListEntries: { [favoritesListId: string]: any };  /** Containts a mapping favorite list id -> List of all its entries with metadata */
...
export class FavoritesState {
  constructor(private favoritesApiService: FavoritesService, private snackbar: ColidMatSnackBarService) { }

  @Selector()
  public static getFavorites(state: FavoritesStateModel): FavoriteListMetadata[] {
  ...
  public static getFavoriteUrisToListMapping(state: FavoritesStateModel): {
    ...
    uris.forEach(u => {
      mapping[u] = state.favorites.filter(f => f.favoritesListEntries.findIndex(e => e.pidUri == u) > -1).map(f => f.id);
    })
  ...
  @Selector()
  public static getCurrentlyLoadingListIds(state: FavoritesStateModel): string[] {
    return state.currentlyLoadingListIds;
  }

  @Action(FetchFavorites)
  FetchFavorites({ patchState }: StateContext<FavoritesStateModel>, action: FetchFavorites) {
  ...
  SetEntryDetailsMetadata({ patchState }: StateContext<FavoritesStateModel>, action: SetEntryDetailsMetadata) {
    patchState({
      selectedEntry: {
  ...
  SaveFavoriteList(ctx: StateContext<FavoritesStateModel>, action: SaveFavoriteList){
    var payload = { name: action.name };

    return this.favoritesApiService.editFavoritesList(action.usedId, action.favoriteListId, payload).subscribe({
      next: (res) => {
        this.snackbar.success("Favorite List updated", "This list was successfully updated.");
        ctx.setState(
          patch<FavoritesStateModel>({
            favorites: updateItem<FavoriteListMetadata>(f => f.id == action.favoriteListId, 
  ...
  DeleteFavorite(ctx: StateContext<FavoritesStateModel>, action: DeleteFavorite) {
    return this.favoritesApiService
      ...
      .pipe(
        tap((res: MessageDto) => {
          ctx.setState(
            patch({
              favorites: patch(removeItem<FavoriteListMetadata>(f => f.id == action.id)),
            })
          );
        })
      )
  ...
  RemoveEntry(ctx: StateContext<FavoritesStateModel>, action: RemoveEntry) {
    ...
    return this.favoritesApiService
      ...
      .pipe(
        tap((res: MessageDto) => {
          var updatedDocumentsItem = ctx.getState().favoriteListEntries[action.favoriteListId];
          delete updatedDocumentsItem[action.pidUri];
          ctx.setState(
            patch<FavoritesStateModel>({
              favorites: updateItem<FavoriteListMetadata>(f => f.id == action.favoriteListId,
                patch<FavoriteListMetadata>({
                  favoritesListEntries: removeItem(m => m.id == action.favoritesListEntryId)
                })
              ),
  ...
  SaveEntry(ctx: StateContext<FavoritesStateModel>, action: SaveFavoriteListEntry) {
    var payload = { personalNote: action.personalNote };

    return this.favoritesApiService.editFavoritesListEntry(action.userId, action.favoriteListEntryId, payload)
      .subscribe((res) => {
        ...
        ctx.setState(
          patch({
            ...
            favoriteListEntryList: patch({
              [action.favoriteListId]: updateItem<any>(e => e.EntryId[0] == action.favoriteListEntryId, patch(updatedPersonalNote))
            })
