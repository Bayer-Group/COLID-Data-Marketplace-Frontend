...
import { FavoritesState, FetchFavorites, ResetEntryDetailsMetadata, SaveFavoriteListEntry } from "../../favorites.state";
...
@Component({
  ...
  styleUrls: ["../favorite-list.component.scss", "./favorites-open.component.scss"],
})
export class FavoritesOpenComponent implements OnInit, OnDestroy {
  ...
  @Select(FavoritesState.getFavoriteListEntryList) favoriteListEntryList$: Observable<{[favoriteListId: string]: any[]}>;
  ...
  ngOnInit(): void {
    ...
    this.sub.add(this.selectedEntry$.subscribe(r => {
    ...
    this.sub.add(this.authService.currentUserId$.subscribe((uid) => (this.userId = uid)));
    if (this.userId) {
      this.fetchFavorites();
    }
    this.sub.add(this.favorites$.subscribe((f) => {
    }));

    this.sub.add(this.favoriteListEntries$.subscribe((f) => {
      this.favoriteListEntries = f;
      this.loadEntryInfo(this.favoritesListId, this.pidUri);
    }));
    this.sub.add(this.favoriteListEntryList$.subscribe((f) => {
      this.favoriteListEntryList = f;
    }));

    this.sub.add(this.metadata$.subscribe(m => {
      this.metadata = m;
      this.loading = false;
    }));
  }
  ...
  loadEntryInfo(favoriteListId: string, pidUri: string){
    if(this.favoriteListEntries && this.favoriteListEntries[favoriteListId] && this.favoriteListEntries[favoriteListId][pidUri]){
      var sourceClone = JSON.parse(JSON.stringify(this.favoriteListEntries[favoriteListId][pidUri]));
  ...
  saveFavoriteListEntry(personalNote: string, favoriteListId: string, favoriteListEntryId: string, pidUri: string) {
    this.store.dispatch(new SaveFavoriteListEntry(this.userId, favoriteListId, favoriteListEntryId, pidUri, personalNote));
    this.toggleEdit();
  }
