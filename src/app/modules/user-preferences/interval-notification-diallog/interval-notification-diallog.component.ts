...
import { DialogData } from 'src/app/components/sidebar/search-filter-dialog/search-filter-dialog.component';
...
import { AddStoredQueryToSearchFiltersDataMarketplace, FetchSearchFilterDataMarketplace, RemoveStoredQueryToSearchFiltersDataMarketplace } from 'src/app/states/user-info.state';
...
export class IntervalNotificationDiallogComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<IntervalNotificationDiallogComponent>,
  ...
  ngOnInit(): void {
    ...
    this.selectedSubscriptionValue= storedQuery?storedQuery.executionInterval:"Daily";
  }

  addStoreQuery(){
    const storedQuery= new StoredQueryDto(this.selectedSubscriptionValue,this.searchFilterDataMarketplaceDto.id)
   this.dialogRef.close(this.store.dispatch(new AddStoredQueryToSearchFiltersDataMarketplace(storedQuery)).subscribe(()=>{
        this.snackBar.success('Added Subscription', 'The selected search has been subscribed successfully.');
      }));
  }
