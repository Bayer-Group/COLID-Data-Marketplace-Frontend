...
export class EntityDisplayItemTaxonomyComponent implements OnInit {
  ...
  get isBadgeTaxonomy(): boolean {
    const key =
      this.metadataProperty == null ? this.key : this.metadataProperty.key;
  ...
  ngOnInit(): void {
    this.taxonomyList$ = this.store
      .dispatch(
        new FetchTaxonomyList(
          this.range ??
      ...
      .pipe(
        concatMap((_) => {
          let taxonomyList = this.store.selectSnapshot(
            TaxonomyState.getTaxonomyList
          );
          let taxonomy = taxonomyList.get(
            this.range ??
              this.metadataProperty.properties[Constants.Metadata.RDFS.Range]
          );
